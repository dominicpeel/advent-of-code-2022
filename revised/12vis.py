...
# 3d voxel plot
voxels = np.zeros((len(grid), len(grid[0]), 26), dtype=bool)
face_colors = np.empty((len(grid), len(grid[0]), 26, 4))
...
for i in range(len(grid)):
    for j in range(len(grid[i])):
        for k in range(int(z[i][j])+1):
            voxels[i][j][k] = True
            face_colors[i][j][k] = cmap(norm(k))

ax = plt.figure().add_subplot(projection='3d')
ax.voxels(voxels, facecolors=face_colors)
ax.set_xlim(0, len(grid))
ax.set_ylim(0, len(grid[0]))
ax.set_zlim(0, 25)
ax.set_box_aspect((np.ptp(X), np.ptp(Y), np.ptp(Z)))

plt.savefig("12_3Dvoxels.png", bbox_inches='tight', pad_inches=0, dpi=300)

# 3d voxel plot with path
for (i, j, k) in path:
    i, j, k = map(int, (i, j, k))
    face_colors[i][j][k] = np.array([1, 0, 0, 1])
...
plt.savefig("12_3Dvoxelspath.png", bbox_inches='tight', pad_inches=0, dpi=300)
